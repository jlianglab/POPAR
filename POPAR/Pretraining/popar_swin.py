...
from datasets import Popar_chestxray, Popar_imagenet, Popar_allXray, build_md_transform
import wandb
import sys
import numpy as np
...
def main(conf):
    ...
    with wandb.init(project=conf.method, config = conf):
        ...
        if conf.dataset =="nih14":
            ...
            train_dataset = Popar_imagenet(conf.train_image_path_file,
                                            build_md_transform(mode="train", dataset="imagenet"), image_size=448,
                                            patch_size=32)
            test_dataset = Popar_imagenet(conf.test_image_path_file,
                                           build_md_transform(mode="validation", dataset="imagenet"), image_size=448,
                                           patch_size=32)
        elif conf.dataset == "allxrays":
            train_dataset = Popar_allXray(augment= build_md_transform(mode="train", dataset="chexray"),
                                          machine=conf.server, views="frontal", image_size=448, patch_size=32)
