...
parser.add_argument('--dataset', dest='dataset', default="jsrt_clavicle", type=str)
...
parser.add_argument('--num_classes', dest='num_classes', type=int,default=1)
...
def train_one_epoch(model,train_loader, optimizer, loss_scaler, epoch ):
    ...
    for idx, (img,mask) in enumerate(train_loader):
        ...
        mask = mask.float().cuda(non_blocking=True).unsqueeze(1)
    ...
    with torch.no_grad():
        ...
        for idx, (img, mask) in enumerate(test_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                mask = mask.unsqueeze(1).cpu().detach()
...
def main(conf):
    train_dataset = JSRTClavicleDataset(conf.train_image_path_file, image_size=(conf.img_size, conf.img_size))
    ...
    val_dataset = JSRTClavicleDataset(conf.val_image_path_file, image_size=(conf.img_size, conf.img_size), mode="val")
    ...
    test_dataset = JSRTClavicleDataset(conf.test_image_path_file, image_size=(conf.img_size, conf.img_size), mode="val")
