...
import math
import sys
from segmentation_datasets import  VinDrRibCXRDataset
import time
...
# assert args.dataset in ["pxs", "montgomery", "jsrt_lung", "jsrt_clavicle", "jsrt_heart","vindrribcxr"]

assert args.dataset in ["vindrribcxr"]
...
def build_model(conf):
    ...
    if conf.weight == "popar_swin_allxrays_448":
        ...
        model = UperNet_swinv2(img_size=512, num_classes=conf.num_classes)
...
def main(conf):
    ...
    train_dataset = VinDrRibCXRDataset(conf.train_image_path_file[0], image_size=(conf.img_size,conf.img_size), mode="train", anno_percent=conf.anno_percent )
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=conf.batch_size,num_workers=conf.num_workers, pin_memory=True, shuffle=True, drop_last= (len(train_dataset)>=conf.batch_size) )

    val_dataset = VinDrRibCXRDataset(conf.val_image_path_file[0],image_size=(conf.img_size,conf.img_size), mode="val", anno_percent=100 )
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=conf.batch_size, num_workers=conf.num_workers,pin_memory=True, shuffle=True,drop_last=False )

    test_dataset = VinDrRibCXRDataset(conf.test_image_path_file[0],image_size=(conf.img_size,conf.img_size), mode="val",anno_percent=100)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=conf.batch_size,num_workers=conf.num_workers, pin_memory=True,drop_last=False, shuffle=False )
