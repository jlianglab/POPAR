...
parser.add_argument('--run', dest='run', default="distillation_model_full_tuning", type=str)
...
def test(model_path, test_loader,conf):
    ...
    test_class_name = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Pleural Effusion']
    class_name = ['No Finding', 'Enlarged Cardiomediastinum', 'Cardiomegaly', 'Lung Opacity',
                  'Lung Lesion', 'Edema', 'Consolidation', 'Pneumonia', 'Atelectasis', 'Pneumothorax',
                  'Pleural Effusion', 'Pleural Other', 'Fracture', 'Support Devices']
    test_class_idx = [class_name.index(c) for c in test_class_name]
    _y_test = copy.deepcopy(y_test[:, test_class_idx])
    _p_test = copy.deepcopy(p_test[:, test_class_idx])
    aurocIndividual = computeAUROC(_y_test, _p_test, len(test_class_name))
...
def main(conf):
    ...
    train_dataset = build_classfication_dataset("ChexPert", conf.train_image_path_file, conf.input_size, conf.anno_percent, mode="train")
    valid_dataset = build_classfication_dataset("ChexPert", conf.valid_image_path_file, conf.input_size, anno_percent = 100, mode="validation")
    test_dataset = build_classfication_dataset("ChexPert", conf.test_image_path_file, conf.input_size, anno_percent = 100, mode="test")
...
if __name__ == '__main__':
    ...
    conf = config.CheXpert_Official_Test(args)
