...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = torch.flatten(x, 1)
        return x
    def forward_features_without_pooling(self, x):
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
...
class SwinModel(nn.Module):
    def __init__(self,img_size = 448, hidden_size = 128, num_classes = 1, window_size = 7, depth=[ 2, 2, 18, 2 ],heads=[ 4, 8, 16, 32 ], device = "gpu"):
        ...
        self.depth = depth
        self.heads = heads
        self.swin_model = _SwinTransformer(img_size=img_size,patch_size=4,in_chans=3,num_classes=0,embed_dim=self.hidden_size,depths=self.depth,num_heads= self.heads,
                                          window_size=window_size,mlp_ratio=4.,qkv_bias=True,qk_scale=None,drop_rate=0,drop_path_rate=0.1,ape=False,patch_norm=True,use_checkpoint=False)
        ...
        #
        #         in_channels=1024,
        #         out_channels=32 ** 2 * self.num_classes, kernel_size=1),
        #     nn.PixelShuffle(32),
        # )
    ...
    def forward(self, img_x):
        ...
        # out = self.conv1(restor_feature)
        # out = self.conv2(out)
        # out = self.conv3(out)
        # out = self.conv4(out)
        # out = self.segmentation_head(out)

        out = self.up1(restor_feature)
        out = self.up2(out)
        out = self.up3(out)
        out = self.up4(out)
        out = self.up5(out)
...
class UperNet_swin(nn.Module):
    ...
    def __init__(self, img_size=448, num_classes=1):
        ...
        self.backbone =  _SwinTransformer(img_size=img_size,patch_size=4,in_chans=3,num_classes=0,embed_dim=128,depths=[ 2, 2, 18, 2 ],
...
class UperNet_swinv2(nn.Module):
    # Implementing only the object path
    def __init__(self, img_size=512, num_classes=1):
        super(UperNet_swinv2, self).__init__()


        self.backbone =  _SwinTransformerV2(img_size=img_size,patch_size=4,in_chans=3,num_classes=0,embed_dim=128,depths=[ 2, 2, 18, 2 ],
                                          num_heads= [ 4, 8, 16, 32 ],window_size=16,mlp_ratio=4.,
                                          qkv_bias=True,qk_scale=None,drop_rate=0,drop_path_rate=0.1,ape=False,patch_norm=True,use_checkpoint=False)
